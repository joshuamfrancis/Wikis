...
This guide sets up a WireGuard VPN tunneled through wstunnel over TCP 443 using an AWS EC2 Ubuntu instance as the server. Two Ubuntu clients connect to each other through this tunnel.
...
Same installation steps as [1b](#1b-install-wireguard-and-wstunnel) above.
...
| Handshake OK but no ping between clients | IP forwarding enabled on server? `sysctl net.ipv4.ip_forward`. iptables FORWARD rule present?                  |
| wstunnel connection refused              | Security group allows TCP 443? EC2 instance has a public IP?                                                   |
